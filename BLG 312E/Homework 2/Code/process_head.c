...
{
    // Unlike malloc in threads, we need to open shared memory space for dynamic allocation
    Product *current = mmap(NULL, sizeof(Product), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    // Product takes the current value of global_ID counter and then global_ID is incremented
    current->product_ID = *global_ID;
    *global_ID += 1;
    // We need to set shared memory mutex locks with PROCESS_SHARED attribute
    pthread_mutexattr_t attr;
    pthread_mutexattr_init(&attr);
    pthread_mutexattr_setpshared(&attr, PTHREAD_PROCESS_SHARED);
    pthread_mutex_init(&current->product_Lock, &attr);
...
{
    // Unlike malloc in threads, we need to open shared memory space for dynamic allocation
    Customer *current = mmap(NULL, sizeof(Customer), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
...
Customer *init_Customer_Random(unsigned int *global_ID)
{
    ...
    *global_ID += 1;
    ...
    // Initializing customer ordered items array using shared memory and setting counter
    current->ordered_Items_Counter = rand_count;
    current->ordered_Items = mmap(NULL, sizeof(Product_Customer) * rand_count, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    ...
    // Unlike threads, we can't share the same heap memory so purchased_items is preallocated with maximum available space
    current->purchased_Items = mmap(NULL, sizeof(Product_Customer) * rand_count, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    current->purchased_Items_Counter = 0;
    // Finally, return current
    return current;
}
